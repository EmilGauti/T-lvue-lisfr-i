...
integer :: i, j, ierr, ft
complex(kind=dp), allocatable, dimension(:,:) :: tmpd, tmpd2, c
complex(kind=dp), allocatable, dimension(:) :: tsum
real(kind=dp):: dt, lamb, k, error
!complex(kind=dp) :: trace, error
open(unit = 69,  file = 'trace.dat',    status = 'new', iostat = ierr)
ierr = 0
ft=4096
...
do j = 1, ft
 write(69,fmt='(I4,2X,E15.8,2X,E15.8)') j, tsum(j)  ! Eigenvalues printed
end do
end program main
